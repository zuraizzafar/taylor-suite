@extends('layouts.app')
@section('title', 'Workers Leaderboard')
@section('page-title', 'Workers Report')
...
<div class="pt-2 max-w-5xl space-y-5">
    ...
    <div class="grid grid-cols-3 gap-4">
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4 text-center">
            <p class="text-xs text-slate-500 mb-1">Total Suits Stitched</p>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4 text-center">
            <p class="text-xs text-slate-500 mb-1">Total Paid Out</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100">
            <h2 class="text-sm font-semibold text-slate-700">Worker Leaderboard</h2>
            <p class="text-xs text-slate-400 mt-0.5">{{ $from }} to {{ $to }} · sorted by suits stitched</p>
        </div>

        @if($workers->isEmpty())
            <p class="text-sm text-slate-400 px-5 py-8 text-center">No workers found.</p>
        @else
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-center">Suits</th>
