...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="report-title">{{ $isUrdu ? 'کاریگروں کی سمری رپورٹ' : 'WORKERS SUMMARY REPORT' }}</div>
    ...
    <div class="kpi-row">
        <div class="kpi-card" style="width:33%">
            ...
            <div class="kpi-val">{{ $totalSuits }} suits</div>
        </div>
        <div class="kpi-card" style="width:33%">
            <div class="kpi-lbl">Total Earned</div>
            <div class="kpi-val" style="color:#2563eb">Rs {{ number_format($totalEarned) }}</div>
        </div>
        <div class="kpi-card" style="width:34%">
            <div class="kpi-lbl">Total Paid Out</div>
            <div class="kpi-val" style="color:#16a34a">Rs {{ number_format($totalPaid) }}</div>
        </div>
    </div>
    ...
    <table class="report-table">
        <thead>
            <tr>
                ...
                <th style="text-align:right">Earned (Rs)</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:center">{{ $worker->period_suits }}</td>
                <td style="text-align:right; font-weight:700; color:#2563eb">Rs {{ number_format($worker->period_earned) }}</td>
                <td style="text-align:right; font-weight:700; color:#16a34a">Rs {{ number_format($worker->total_paid) }}</td>
                <td style="text-align:right; font-weight:700; color:{{ $worker->balance_due > 0 ? '#dc2626' : '#16a34a' }}">Rs {{ number_format($worker->balance_due) }}</td>
            </tr>
            @empty
            <tr><td colspan="7" style="text-align:center; color:#94a3b8; padding:12px">No worker data found.</td></tr>
            @endforelse
        </tbody>
        ...
        <tfoot>
            <tr>
                ...
                <td style="text-align:right; color:#2563eb">Rs {{ number_format($totalEarned) }}</td>
                <td style="text-align:right; color:#16a34a">Rs {{ number_format($totalPaid) }}</td>
                <td style="text-align:right; color:#dc2626">Rs {{ number_format($workers->sum('balance_due')) }}</td>
            </tr>
        </tfoot>
        @endif
    </table>
