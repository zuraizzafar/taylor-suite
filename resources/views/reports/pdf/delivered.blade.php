...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .kpi-card { display: table-cell; background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 6px; padding: 8px; text-align: center; }
    .kpi-lbl { font-size: 7.5px; font-weight: 700; text-transform: uppercase; color: #047857; margin-bottom: 2px; }
...
@php
    ...
    $totalDelivered = $suits->count();
    $grossWorth     = $suits->sum(fn($s) => (float) ($s->order->total_amount ?? 0));
@endphp
...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="report-title">{{ $isUrdu ? 'ترسیل شدہ سوٹ رپورٹ' : 'DELIVERED SUITS REPORT' }}</div>
    ...
    <div class="kpi-row">
        <div class="kpi-card" style="width:50%">
            <div class="kpi-lbl">Total Delivered Suits</div>
            <div class="kpi-val">{{ $totalDelivered }} items</div>
        </div>
        <div class="kpi-card" style="width:50%">
            <div class="kpi-lbl">Calculated Gross Worth</div>
            <div class="kpi-val">Rs {{ number_format($grossWorth) }}</div>
        </div>
    </div>
    ...
    <table class="report-table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $idx + 1 }}</td>
                <td>{{ $suit->delivered_at?->format('d M Y, h:i A') ?? '—' }}</td>
                <td style="font-weight:700">{{ $suit->suit_code }}</td>
                <td>{{ $suit->order?->order_number ?? '—' }}</td>
                <td>{{ $suit->customer->name }}</td>
                <td>{{ $suit->worker?->name ?? '—' }}</td>
                <td style="text-align:right">Rs {{ number_format($suit->order?->total_amount ?? 0) }}</td>
            </tr>
            @empty
            <tr><td colspan="7" style="text-align:center; color:#94a3b8; padding:12px">No delivered suits in this date range.</td></tr>
            @endforelse
        </tbody>
        @if($suits->isNotEmpty())
        <tfoot>
            <tr>
                <td colspan="6" style="text-align:right">Total Items / Gross Worth:</td>
                <td style="text-align:right">Rs {{ number_format($grossWorth) }}</td>
            </tr>
        </tfoot>
        @endif
    </table>
