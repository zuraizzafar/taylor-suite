...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .kpi-card { display: table-cell; background: #fef2f2; border: 1px solid #fecca3; border-radius: 6px; padding: 8px; text-align: center; }
    .kpi-lbl { font-size: 7.5px; font-weight: 700; text-transform: uppercase; color: #991b1b; margin-bottom: 2px; }
    .kpi-val { font-size: 14px; font-weight: 800; color: #991b1b; }
    ...
    table.report-table tfoot td { padding: 6px 8px; background: #fef2f2; border-top: 1.5px solid #fca5a5; font-weight: 700; color: #991b1b; }
...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="report-title">{{ $isUrdu ? 'واجبات كى رپورٹ' : 'PENDING BALANCES RECEIVABLES' }}</div>
            <div class="report-meta">As of: <strong>{{ now()->format('d M Y, h:i A') }}</strong></div>
        </div>
    </div>

    <div class="kpi-row">
        <div class="kpi-card" style="width:50%">
            <div class="kpi-lbl">Customers with Pending Dues</div>
            <div class="kpi-val">{{ $customers->count() }} customers</div>
        </div>
        <div class="kpi-card" style="width:50%">
            <div class="kpi-lbl">Total Outstanding Balance</div>
            <div class="kpi-val">Rs {{ number_format($grandTotal) }}</div>
        </div>
    </div>
    ...
    <table class="report-table">
        <thead>
            <tr>
                ...
                <th style="text-align:right">Total Outstanding Due</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                <td>{{ $idx + 1 }}</td>
                <td style="font-weight:700">{{ $customer->file_number }}</td>
                <td>{{ $customer->name }}</td>
                <td>{{ $customer->mobile }}</td>
                <td style="text-align:center">{{ $customer->orders->count() }} order(s)</td>
                <td style="text-align:right; font-weight:700; color:#dc2626">Rs {{ number_format($customer->total_outstanding) }}</td>
            </tr>
            @empty
            <tr><td colspan="6" style="text-align:center; color:#94a3b8; padding:12px">No outstanding balances found.</td></tr>
            @endforelse
        </tbody>
        @if($customers->isNotEmpty())
        <tfoot>
            <tr>
                <td colspan="5" style="text-align:right">Grand Total Outstanding Dues:</td>
                <td style="text-align:right; font-size:12px; font-weight:800">Rs {{ number_format($grandTotal) }}</td>
            </tr>
        </tfoot>
        @endif
    </table>
