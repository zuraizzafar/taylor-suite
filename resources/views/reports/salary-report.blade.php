...
<div class="max-w-6xl space-y-5 pt-2">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden" x-data="{ open: false }">
        <div class="flex items-center px-5 py-3 cursor-pointer hover:bg-slate-50 select-none" @click="open = !open">
            <div class="flex-1">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('workers.report', $worker) }}" onclick="event.stopPropagation()" class="text-xs text-blue-600 hover:underline">View →</a>
        ...
        <div x-show="open" x-cloak class="border-t border-slate-100">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-0 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                ...
                <div class="p-4">
                    ...
                    <div class="space-y-1.5 mb-3 max-h-40 overflow-y-auto">
                        ...
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <form method="POST" action="{{ route('workers.salary-payments.destroy', $sp) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete this payment?')" class="text-red-400 hover:text-red-600 text-[11px]">✕</button>
                            </form>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <form method="POST" action="{{ route('workers.salary-payments.store', $worker) }}" class="bg-slate-50 rounded-lg p-3 space-y-2">
                        @csrf
                        ...
                        <div class="flex gap-2">
                            <input type="number" name="amount_paid" min="1" step="0.01" placeholder="Amount (Rs)"
                                value="{{ $worker->balance_due > 0 ? round($worker->balance_due) : '' }}" required
                        ...
                        <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white text-xs font-semibold py-1.5 rounded-lg">
                            ✓ {{ __('Mark Paid') }}
                        </button>
                    </form>
                </div>
