...
<div class="max-w-6xl space-y-5 pt-2">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
        <form method="GET" action="{{ route('reports.salary-report') }}" class="flex flex-wrap items-end gap-3">
            ...
            <button class="bg-slate-700 hover:bg-slate-800 text-white text-sm font-medium px-4 py-1.5 rounded-lg">Filter</button>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        @foreach([
            ['label' => 'Suits Stitched',  'value' => $totalSuits,             'color' => 'blue',  'prefix' => ''],
            ['label' => 'Total Earned',     'value' => 'Rs '.number_format($totalEarned), 'color' => 'indigo', 'prefix' => ''],
            ['label' => 'Paid This Period', 'value' => 'Rs '.number_format($totalPaid),   'color' => 'green',  'prefix' => ''],
            ['label' => 'Balance Due',      'value' => 'Rs '.number_format($totalBalance),'color' => $totalBalance > 0 ? 'red' : 'slate', 'prefix' => ''],
        ] as $card)
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden"
        ...
        <div x-show="open" x-cloak class="border-t border-slate-100">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-0 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                ...
                <div class="p-4">
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">By Stitch Type</p>
                ...
                <div class="p-4">
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Salary Payments</p>

                    @if($worker->salaryPayments->isEmpty())
                        <p class="text-xs text-slate-400 mb-3">No payments recorded.</p>
                    @else
                    ...
                    <form method="POST" action="{{ route('workers.salary-payments.store', $worker) }}"
                        ...
                        <p class="text-xs font-semibold text-slate-600">Record Payment</p>
    ...
    @if($workers->every(fn($w) => $w->period_suits === 0 && $w->balance_due == 0))
    <div class="text-center text-sm text-slate-400 py-10">No salary data for this period.</div>
    @endif
