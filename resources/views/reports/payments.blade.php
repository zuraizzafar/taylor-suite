...
<div class="pt-2 space-y-5">
    ...
    <form method="GET" action="{{ route('reports.payments') }}"
        ...
        <div>
            <label class="block text-xs font-medium text-slate-600 mb-1">{{ __('Method') }}</label>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('reports.payments-pdf', request()->all()) }}" target="_blank"
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="md:col-span-2 bg-green-50 border border-green-100 rounded-xl p-4">
            ...
            <p class="text-xs text-green-500 mt-1">{{ $payments->count() }} {{ __('payment(s)') }} · {{ $from }} to {{ $to }}</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-slate-600">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-medium">{{ __('Order Number') }}</th>
                    <th class="px-4 py-3 text-left font-medium">{{ __('Customer') }}</th>
                    <th class="px-4 py-3 text-left font-medium">{{ __('Amount') }}</th>
                    <th class="px-4 py-3 text-left font-medium">{{ __('Method') }}</th>
                    <th class="px-4 py-3 text-left font-medium">{{ __('Reference') }}</th>
                    <th class="px-4 py-3 text-left font-medium">{{ __('Received By') }}</th>
                </tr>
            </thead>
            ...
            <tfoot class="bg-slate-50">
                <tr>
                    <td colspan="3" class="px-4 py-2 font-semibold text-slate-700 text-right">{{ __('Total') }}:</td>
