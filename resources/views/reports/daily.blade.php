...
@section('content')
<div class="pt-2">
    <form method="GET" action="{{ route('reports.daily') }}" class="flex flex-wrap gap-2 mb-5 items-center">
        ...
        <a href="{{ route('reports.export-csv', array_merge(['report' => 'daily'], request()->all())) }}" 
           class="bg-emerald-600 hover:bg-emerald-700 text-white text-sm px-4 py-2 rounded-lg flex items-center gap-1 font-semibold">
            📥 {{ __('Export Excel') }}
        </a>
        <a href="{{ route('reports.daily-pdf', request()->all()) }}" target="_blank"
           class="bg-slate-700 hover:bg-slate-800 text-white text-sm px-4 py-2 rounded-lg flex items-center gap-1 font-semibold">
            📄 {{ __('Print PDF') }}
        </a>
    </form>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-slate-600">
                <tr>
                    <th class="px-4 py-2 text-left font-medium">{{ __('Order Number') }}</th>
                    ...
                    <th class="px-4 py-2 text-left font-medium">{{ __('Total') }}</th>
