...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .info-row, .payment-wrap, .note-box, .legal-box, .copy-label-bar, table.items {
...
<div class="page">
    ...
    <div class="header">
        <div class="header-left">
            ...
            @php $ntn = $settings['company_tax_number'] ?? ''; @endphp
            @if($ntn || !empty($tax['registration_no']))
            <div class="company-meta" style="color:#1e293b;font-weight:700;">
                @if($ntn)NTN: {{ $ntn }}@endif
                @if(!empty($tax['registration_no']))@if($ntn) &nbsp;|&nbsp; @endif{{ $tax['label'] }} Reg. No.: {{ $tax['registration_no'] }}@endif
            </div>
            @endif
        </div>
    ...
    <div class="payment-wrap" style="display: table; width: 100%; margin-bottom: 16px;">
        ...
        <div class="payment-wrap-inner" style="display: table-cell; vertical-align: top; text-align: right; width: 50%;">
            <table class="payment-table" style="display: inline-table; width: 230px; border-collapse: collapse;">
                @include('tax._rows', ['doc' => $fabricSale, 'taxLabel' => $tax['label']])
                <tr>
                ...
                <tr class="row-grand">
                    ...
                    <td class="val" style="font-family:DejaVu Sans,sans-serif;color:#fbbf24;font-weight: 700;font-size:11.5px">Rs {{ number_format($fabricSale->total_amount) }}</td>
                </tr>
            </table>
        </div>
    </div>
