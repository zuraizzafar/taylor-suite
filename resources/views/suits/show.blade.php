...
<div class="pt-2 space-y-6">
            ...
            <div class="text-right">
                ...
                <div class="flex gap-2 mt-2 justify-end">
                    ...
                    <a href="{{ route('suits.tag', $suit) }}" target="_blank"
                       class="text-xs bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded-lg">{{ __('Print Tag') }}</a>
    ...
    {{-- Status control --}}
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
        <h3 class="font-semibold text-slate-700 mb-4">📋 {{ __('Status') }}</h3>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
        <h3 class="font-semibold text-slate-700 mb-3">👨‍🔧 {{ __('Assign Worker') }}</h3>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
        <h3 class="font-semibold text-slate-700 mb-3">{{ __('QR Code') }}</h3>
        <div class="flex items-start gap-6">
            <img src="{{ Storage::url($suit->qr_code_path) }}" alt="QR Code" class="w-32 h-32">
            <div class="flex-1">
                <p class="text-sm text-slate-600 mb-2">{{ __('Scan to open the public status page.') }}</p>
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="copyPublicLink(document.getElementById('copy-link-btn'))"
                        class="text-xs bg-slate-700 hover:bg-slate-800 text-white px-3 py-1.5 rounded-lg whitespace-nowrap">{{ __('Copy') }}</button>
                </div>
                <a href="{{ route('scan.show', $suit->suit_code) }}" target="_blank"
                    class="mt-2 inline-block text-xs text-blue-600 hover:underline">{{ __('Open in new tab') }}</a>
            </div>
        </div>
    </div>
    @endif
